...
class MasterController extends Controller {
    ...
    public function index(Request $request, string $type) { $model=$this->model($type); $search=$request->string('search')->trim()->toString(); $records=$model::when($search, fn($query)=>$query->where(fn($q)=>$q->where('name','like',"%{$search}%")->orWhere('category','like',"%{$search}%")->orWhere('description','like',"%{$search}%")))->latest()->get(); return view('masters.index', compact('type','records','search')); }
